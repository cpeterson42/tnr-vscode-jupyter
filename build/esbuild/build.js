...
const bundleConfig = (0, common_1.getBundleConfiguration)();
...
function style({ minify = true, charset = 'utf8' } = {}) {
    return {
        name: 'style',
        setup({ onResolve, onLoad }) {
            ...
            onResolve({ filter: /\.css$/, namespace: 'file' }, (args) => {
                ...
                const resolved = fs_extra_1.default.existsSync(absPath) ? relPath : args.path;
...
function createConfig(source, outfile, target, watch) {
    const inject = [];
    if (target === 'web') {
    ...
    return {
        ...
        inject,
        plugins: target === 'desktop' ? [] : [style(), (0, esbuild_plugin_less_1.lessLoader)()],
        loader: target === 'desktop' ? {} : loader
    };
}
async function build(source, outfile, options) {
    if (options.watch) {
        ...
        const size = fs_extra_1.default.statSync(outfile).size;
        const relativePath = `./${path.relative(extensionFolder, outfile)}`;
        console.log(`asset ${(0, colors_1.green)(relativePath)} size: ${(size / 1024).toFixed()} KiB`);
        if (isDevbuild && result.metafile) {
            const metafile = `${outfile}.esbuild.meta.json`;
            await fs_extra_1.default.writeFile(metafile, JSON.stringify(result.metafile));
            console.log(`metafile ${(0, colors_1.green)(`./${path.relative(extensionFolder, metafile)}`)}`);
        }
    }
}
async function buildAll() {
    ...
    const getLessBuilders = (watch = isWatchMode) => {
        return [
            build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'interactive-common', 'variableExplorerGrid.less'), path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'interactive-common', 'variableExplorerGrid.css'), { watch, target: 'web' }),
            build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'react-common', 'seti', 'seti.less'), path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'react-common', 'seti', 'seti.css'), { watch, target: 'web' })
        ];
    };
    ...
    builders.push(build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'ipywidgets', 'kernel', 'index.ts'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'ipywidgetsKernel', 'ipywidgetsKernel.js'), { target: 'web', watch: watchAll }), build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'ipywidgets', 'renderer', 'index.ts'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'ipywidgetsRenderer', 'ipywidgetsRenderer.js'), { target: 'web', watch: watchAll }), build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'variable-view', 'index.tsx'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'viewers', 'variableView.js'), { target: 'web', watch: watchAll }), build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'plot', 'index.tsx'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'viewers', 'plotViewer.js'), { target: 'web', watch: watchAll }), build(path.join(extensionFolder, 'src', 'webviews', 'webview-side', 'data-explorer', 'index.tsx'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'viewers', 'dataExplorer.js'), { target: 'web', watch: watchAll }));
    if (isDevbuild) {
        builders.push(build(path.join(extensionFolder, 'src', 'test', 'datascience', 'widgets', 'rendererUtils.ts'), path.join(extensionFolder, 'dist', 'webviews', 'webview-side', 'widgetTester', 'widgetTester.js'), { target: 'web', watch: watchAll }));
    }
    if (bundleConfig !== 'desktop') {
        builders.push(build(path.join(extensionFolder, 'src', 'extension.web.ts'), path.join(extensionFolder, 'dist', 'extension.web.bundle.js'), { target: 'web', watch: watchAll }));
    }
    if (bundleConfig !== 'web') {
        builders.push(build(path.join(extensionFolder, 'src', 'extension.node.ts'), path.join(extensionFolder, 'dist', 'extension.node.js'), { target: 'desktop', watch: isWatchMode }));
        builders.push(build(path.join(extensionFolder, 'src', 'extension.node.proxy.ts'), path.join(extensionFolder, 'dist', 'extension.node.proxy.js'), 
        // This file almost never ever changes, hence no need to watch this.
        { target: 'desktop', watch: false }));
        builders.push(...deskTopNodeModulesToExternalize
            ...
            return build(fullPath, path.join(extensionFolder, 'dist', 'node_modules', `${path.join(module, 'index.js')}`), {
                target: 'desktop',
                // These almost never change, easier to re-run copmilation if packges change.
                watch: false
            });
        }));
        builders.push(copyJQuery(), copyAminya(), copyZeroMQ(), copyZeroMQOld(), copyNodeGypBuild(), buildVSCodeJsonRPC());
    }
...
async function copyJQuery() {
    ...
    await fs_extra_1.default.ensureDir(path.dirname(target));
    await Promise.all([
        fs_extra_1.default.copyFile(source, target),
        fs_extra_1.default.copyFile(license, path.join(extensionFolder, 'dist', 'node_modules', 'jquery', 'LICENSE.txt'))
    ]);
}
async function copyAminya() {
    ...
    await fs_extra_1.default.copy(source, target, { recursive: true });
}
...
module.exports = require('./index');`;
    await fs_extra_1.default.writeFile(path.join(path.dirname(target), 'node.js'), contents);
...
function shouldCopyFileFromZmqFolder(resourcePath) {
    ...
    if (fs_extra_1.default.statSync(resourcePath).isDirectory()) {
        return true;
    }
    // return true;
    ...
    if (foldersToIgnore.some((folder) => resourcePath.toLowerCase().startsWith(path.join(parentFolder, folder).replace(/\\/g, '/').toLowerCase()))) {
        return false;
    }
    ...
    const preBuildsFoldersToCopy = (0, common_1.getZeroMQPreBuildsFoldersToKeep)();
